...
const mockUpsertProfile = vi.fn().mockResolvedValue({ phone: '+1234', name: 'Test', email: 'test@test.com' });
...
describe('POST /profile validation', () => {
  ...
  it(`rejects email exceeding ${EMAIL_MAX_LENGTH} characters`, async () => {
    const { req, res } = mockReqRes({ name: 'John', email: 'a'.repeat(EMAIL_MAX_LENGTH - 8) + '@test.com' });
    await postProfileHandler(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
  });
